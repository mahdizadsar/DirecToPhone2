...
// 	GPIOE -> ODR |= (1 << 7);
	...
	GPIOC -> ODR &= ~(1 << 8);				//Releas PHY Powerdown  
	GPIOC -> ODR &= ~(1 << 6);				//Reset PHY 
	GPIOC -> ODR |= (1 << 6);				//Release PHY Reset
...
///////////////////////////////
  ...
  ETH-> DMAOMR |= ETH_DMAOMR_FEF | ETH_DMAOMR_FUGF;
  ...
  ETH->MACFFR = MFFR_HPF | MFFR_PAM | MFFR_RA;
...
static void write_PHY (U32 PhyReg, U16 Value) {
  ...
  ETH->MACMIIDR = Value;
  ETH->MACMIIAR = PHY_DEF_ADDR << 11 | PhyReg << 6 | MMAR_MW | MMAR_MB | 4 << 2;
...
static U16 read_PHY (U32 PhyReg) {
  ...
  ETH->MACMIIAR = PHY_DEF_ADDR << 11 | PhyReg << 6 | MMAR_MB | 4 << 2;
